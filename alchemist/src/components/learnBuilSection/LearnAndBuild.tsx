...
const LearnAndBuild: React.FC = () => {
  ...
  <p className="text-gray text-sm sm:text-base">
    Master the art of prompt engineering
      ...
      {/* Tab Navigation */}
      {/* <div className="px-1 py-1 rounded-xl mb-6">
        <div className="flex flex-col  sm:flex-row w-full rounded-lg overflow-hidden text-sm font-medium text-gray">
          <button
            ...
            className={`flex-1 text-center px-4 sm:px-6 py-2 transition-colors rounded-xl ${
              activeTab === "rules"
                ? "bg-white text-black"
                : "bg-gold text-black hover:text-gray"
            }`}
          >
            AICHEMIST Blueprints
          </button>
          <button
...
<div className="px-1 py-1 rounded-xl mb-6">
  <div className="flex flex-col sm:flex-row w-full rounded-lg overflow-hidden text-sm font-medium text-gray gap-2">
    <button
      onClick={() => setActiveTab("rules")}
      className={`flex-1 text-center px-4 sm:px-6 py-2 transition-colors rounded-xl ${
        activeTab === "rules"
