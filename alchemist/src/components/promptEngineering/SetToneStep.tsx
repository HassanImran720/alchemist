...
const SetToneStep: React.FC<SetToneStepProps> = ({ toneData, setToneData }) => {
  ...
  const ToneBankModal = React.memo(() => (
    <Modal onClose={closeModal}>
      <div className="px-4 sm:px-6 pb-6">
        <h2 className="text-lg sm:text-2xl font-semibold text-black mb-2">Tone Bank</h2>
        <p className="text-sm sm:text-base text-black mb-6">
          Select multiple tone options to define your voice
        </p>
        <div className="flex flex-wrap gap-2 sm:gap-3 mb-4">
          {TONE_OPTIONS.map((tone) => (
            <button
              key={tone}
              type="button"
              onClick={() => toggleTone(tone, tempToneData, setTempToneData)}
              className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg border text-xs sm:text-sm transition-colors ${
                tempToneData.includes(tone)
                  ? " text-black border-gold"
                  : "text-gray-700 border-gold/30 hover:bg-gold hover:text-white"
              }`}
            >
              {tempToneData.includes(tone) && <Check className="w-3 h-3 sm:w-4 sm:h-4" />}
              {tone}
            </button>
          ))}
        </div>
  ...
  // 🔹 Brand Voice Modal
  const BrandVoiceModal = React.memo(() => {
    const [tempBrandVoice, setTempBrandVoice] = useState(brandVoiceDescription);
    ...
    return (
      <Modal onClose={closeModal}>
        <div className="px-4 sm:px-6 pb-6">
          <h2 className="text-lg sm:text-2xl font-semibold text-black mb-2">Brand Voice</h2>
          <p className="text-sm sm:text-base text-gray-600 mb-6">Define your brand's unique voice</p>
          <textarea
            ref={brandVoiceTextareaRef}
            value={tempBrandVoice}
            onChange={(e) => setTempBrandVoice(e.target.value)}
            placeholder="Describe your brand's voice..."
            className="p-2 sm:p-3 w-full border border-gold/30 rounded-md bg-ivory text-sm sm:text-base focus:outline-gold resize-none"
            rows={5}
          />
          <div className="flex justify-end gap-3 mt-4">
            <button onClick={closeModal} className="px-4 py-2 rounded-md border border-gray-300 hover:bg-gray-100">
              Cancel
            </button>
            <button
              onClick={() => {
                setBrandVoiceDescription(tempBrandVoice);
                setActiveModal(null);
              }}
              className="px-4 py-2 rounded-md bg-gold text-white hover:bg-gold/90"
            >
  ...
  // 🔹 Custom Voice Modal
  const CustomVoiceModal = React.memo(() => {
    const [tempCustomTone, setTempCustomTone] = useState(customTone);
    ...
    return (
      <Modal onClose={closeModal}>
        <div className="px-4 sm:px-6 pb-6">
          <h2 className="text-lg sm:text-2xl font-semibold text-gray-900 mb-2">Custom Voice</h2>
          <p className="text-sm sm:text-base text-gray-600 mb-6">Add your own custom tone</p>
          <input
            type="text"
            ref={customToneInputRef}
            value={tempCustomTone}
            onChange={(e) => setTempCustomTone(e.target.value)}
            placeholder="Enter custom tone..."
            className="w-full px-2 sm:px-3 py-2 border border-gold/30 rounded-md bg-ivory text-sm sm:text-base focus:outline-gold"
          />
          <div className="flex justify-end gap-3 mt-4">
    ...
    <div className="rounded-xl border border-gold/30 p-2 md:p-4 mb-4">
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mb-6">
        {["toneBank", "brandVoice", "customVoice"].map((type, i) => {
          const Icon = type === "toneBank" ? Music : type === "brandVoice" ? User : Edit;
          const label =
            type === "toneBank" ? "Tone Bank" : type === "brandVoice" ? "Brand Voice" : "Custom Voice";
          return (
            <button
              key={i}
              type="button"
