...
//     customFields?: any[]; // loosely typed because it comes from user input
...
//       // Safely handle customFields array of strings
...
//   return (
//     <div className="rounded-lg shadow-sm border-[0.5px] border-gold/30 p-6 mb-6 bg-ivory">
...
//       <div className="bg-ivory border-[0.5px] border-gold/30 rounded-md p-4 mb-4 font-mono text-sm text-gray-800 whitespace-pre-wrap min-h-[200px]">
//         {generatePromptText()}
//       </div>
...
//         <button
//           onClick={handleCopy}
//           className="inline-flex items-center px-4 py-2 border-[0.5px] border-gold/30 rounded-md bg-ivory text-sm font-medium text-gray"
//         >
//           {copied ? (
//             <>
...
interface GeneratedPromptProps {
  ...
  references?: string;
  outputFormat?: string;
  onPromptGenerated?: (prompt: string) => void; // Add this prop
}
...
const GeneratedPromptStep: React.FC<GeneratedPromptProps> = ({
  ...
  references = '',
  outputFormat = '',
  onPromptGenerated // Add this prop
}) => {
  const [copied, setCopied] = useState(false);
  const [saved, setSaved] = useState(false);
  ...
  // Call onPromptGenerated when component mounts or when the prompt changes
  useEffect(() => {
